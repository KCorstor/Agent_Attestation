...
Open **http://127.0.0.1:3333**. Use **Run all steps** to reset, start the merchant server, and run **`agent.mjs`** end-to-end (no separate “prepare” step). **Run this step** appears only for **Agent starts checkout** if you want to re-run the agent alone after a full run. **Previous / Next** browse phases. Copy lives in **`demo-ui/steps.json`** (`runnable` marks which phases you can trigger individually vs outcomes of the agent run).
